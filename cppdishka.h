...
class Provider {
public:
    depends<std::unique_ptr<Ts>...> _deps;
public:
    template<typename T>
...
#define request_provide(T, VAL) decltype(auto) provide(cppdishka::provider_t<T>) const { return VAL ; }
#define app_provide(T, VAL) decltype(auto) provide(cppdishka::provider_t<T>) const { \
     if (val = std::get<std::unique_ptr<T>>(this->_deps); val == nullptr) std::get<std::unique_ptr<T>>(this->_deps) = VAL ; \
     return std::get<std::unique_ptr<T>>(this->_deps).get(); }
}